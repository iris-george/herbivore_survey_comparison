########## HERBIVORE SURVEY COMPARISON: LARGE SPECIES ##########
########## 
##########
# This file subsets available survey data to look specifically at herbivorous 
# fish species > 16cm in length (size bins 4-6) and compares their density 
...
# The following subsets the herbivore dataframe for large-bodied species (size 
# bins 4-6).

# subset for large species
large_herbivores <- herbivores[herbivores$size_bin %in% c(4, 5, 6),]


# Large Herbivore Global Model =================================================

# The following section creates a global linear mixed effects model to compare 
# large herbivore density differences between SVC and transect surveys in 
...
# full model 
large_herbivore_global <- lme(log_difference~habitat+octocoral+stony+relief_cm
...
summary(large_herbivore_global)

# VIF values 
vif(large_herbivore_global)
# no collinearity 


# Large Herbivore Dredge =======================================================
...
# dredge
large_herbivore_dredge <- dredge(large_herbivore_global)
large_herbivore_dredge

# subset for delta AIC < 4
large_herbivore_dredge_sub <- subset(large_herbivore_dredge, delta < 4)

# model average of top models 
large_herbivore_model_avg <- model.avg(large_herbivore_dredge_sub)
summary(large_herbivore_model_avg)

# confidence intervals of top model covariates
confint(large_herbivore_model_avg)

# save dredge outputs 
saveRDS(large_herbivore_dredge, here("./outputs/large_herbivore_dredge.rds"))
...
# reef type boxplot
ggplot(large_herbivores, aes(x = habitat, y = log_difference, 
                             fill = habitat)) +
  geom_boxplot() +
...
large_herbivores$cryptic_char <- 
  as.character(large_herbivores$cryptic_behaviour)
ggplot(large_herbivores, aes(x = cryptic_char, y = log_difference, 
                             fill = cryptic_char)) +
  geom_boxplot() +
...
# shape boxplot
ggplot(large_herbivores, aes(x = shape, y = log_difference, fill = shape)) +
  geom_boxplot() +
  ...
  geom_hline(yintercept = 0,
             linetype = "dashed",
             colour = "grey40")
# fusiform significantly positive